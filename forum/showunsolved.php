...
{
 case "UNSOLVED":
 $sql1="SELECT * FROM issues  WHERE STATUS='unsolved' ORDER BY I_D ".$q." LIMIT ".$offset.", 50";
 break;
 case "ID": 
$sql1="SELECT * FROM issues  WHERE STATUS='unsolved' ORDER BY I_D ".$q." LIMIT ".$offset.", 50";
 break;
 case "VOTES": 
$sql1="SELECT * FROM issues  WHERE STATUS='unsolved' ORDER BY VOTES ".$q." LIMIT ".$offset.", 50";
 break;
 case "ANSWER": 
$sql1="SELECT * FROM issues  WHERE STATUS='unsolved' ORDER BY ANSWERS ".$q." LIMIT ".$offset.", 50";
 break;
 case "KNOWLEDGE": 
$sql1="SELECT * FROM issues  WHERE STATUS='unsolved' ORDER BY KNOWLEDGE ".$q." LIMIT ".$offset.", 50";
 break;
 case "DATE": 
$sql1="SELECT * FROM issues  WHERE STATUS='unsolved' ORDER BY DATE ".$q." LIMIT ".$offset.", 50";
 break;
 }
$sql2="SELECT COUNT(*) AS num FROM issues WHERE STATUS='unsolved'";
...
$numunsolvedissue = $count['num'];
echo "<div style='position:absolute;top:242px;left:1010px;'>Total number of UNSOLVED ISSUES = $numunsolvedissue</div>";
...
echo "<div style='position:absolute;width:30px;cursor:pointer;margin-top:10px;left:30%;background:;color:#cadae6;text-decoration:underline;'>
      <span onclick='showunsolved(\"$q\",\"$r\",\"$first\")'>First</span></div>";
	  }
	  
	  
if($numunsolvedissue >=($offset)+50)
{
...
echo "<div style='position:absolute;width:32px;cursor:pointer;margin-top:10px;left:40%;background:;color:#cadae6;text-decoration:underline;'>
      <span onclick='showunsolved(\"$q\",\"$r\",\"$next\")'>Next</span></div>";
}
...
if ($page!=0)
{
	  ...
	  <span onclick='showunsolved(\"$q\",\"$r\",\"$previous\")'>Previous</span>
	  </div>";
}
...
echo "<div style='position:absolute;width:30px;cursor:pointer;margin-top:10px;left:61%;background:;color:#cadae6;text-decoration:underline;'>
      <span onclick='showunsolved(\"$q\",\"$r\",\"$last\")'>Last</span></div>";
	  }
echo "</div>";	  
